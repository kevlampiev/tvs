...
class StoredProcedureMarkTaskAsDone extends Migration
{
    ...
    public function up()
    {
        ...
        DB::unprepared('
                    DROP PROCEDURE IF EXISTS po_mark_task_as_canceled;
        ');
...
SQL;
        ...
        $sql = <<<SQL
CREATE PROCEDURE po_mark_task_as_canceled(IN task_id int)
BEGIN
	call fetch_subtree_ids ('tasks', 'id', 'parent_task_id', task_id, 30, true, @a);
	SET @stm = CONCAT('update tasks set terminate_status="cancel", terminate_date=CURRENT_DATE() where id in (',@a,')');
	PREPARE fetch_childs FROM @stm;
	        EXECUTE fetch_childs;
	        DROP PREPARE fetch_childs;
END;
